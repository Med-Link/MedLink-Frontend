      ...
      .then(res =>{
        const results =  res.data.result;
         ...
         results.forEach(element=>{
          let arr=[];
          arr.push(element.name,element.email,element.contactnumber,element.location,<Button color="primary" round onClick={()=>handleClickOpen(element.document1,element.document2,element.document3)}>View</Button>,<Switch color="primary" inputProps={{ 'aria-label': 'primary checkbox' }} />,<Button color="default" round onClick={handleClickOpenReject}>Reject</Button>);
          array.push(arr);
         })         
        setData(array);
      })
